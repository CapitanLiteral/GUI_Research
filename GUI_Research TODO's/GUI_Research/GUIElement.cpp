...
void GUIElement::AddAnimationOrTransition(gui_events eventToReact, staticAnim_or_transition animOrTransition)
{
	...
	{
		//1.1 If we find that that event has already been told to react we overide the staticAnim_or_transition.---

		//-----
	}
	else
	{
		//1.2 If we doont find that event reaction setted already we just set it.---
...
void GUIElement::OnGuiEvent(int64_t eventToReact)
{
	...
	if (this->eventsToReact & eventToReact)
	{
		//2.1 Search in the map to get the iterator with the value desired.
		std::map<gui_events, staticAnim_or_transition>::iterator it = transAndAnimations.end();

		//---
		//Remember:
		//	Use map find to search in the map. This method return an iterator. More info here: http://www.cplusplus.com/reference/map/map/find/
		//	Map iterator have to values first and second, the first referes to the unique value key. 
...
void GUIElement::ShakeSA(float dt)
{
	...
	//5.2 Update the time counter every frame (currentAnimeTime)
	//----
	...
	if (currentAnimTim < animTime)
	{
		//5.3 Set drawRect.x to rect.x - 25 * the value you get from the curve. use a SHAKE curve.

		//---
	}
	else
	{
		//5.4 When the animation is over reset the drawRect position using the rect and make sure you left the currentAnimation variable to none.
...
void GUIElement::FadeT(float dt)
{
	...
	if (!doingTransition)
	{
		//3.1 If doingTransition is false it means it's the first time FadeT is called so we must set the starting variables. 
	...
	{
		//3.2 If time is less than duration trnsition is still on so get the alpha variation using curves. Use a slow midle curve type. The vaule you get from the curve will be the alpha variation for this frame.
	...
	{
		//3.3 If time is greater than the duration the transition is over so now set currentTransition to none and set active to false if needed.
...
void GUIElement::DropT(float dt)
{
	...
	if (!doingTransition)
	{
		if (mustDisable)
		{
			//4.1 There are two iPoint variables called transOrigin and transDestination. Use them to set the goal of the transition and the origin. 
		...
		{
			//4.2 If we are on an enable case swap origin and the goal.
	...
	if (currentTransTime <= transDuration)
	{
		if (mustDisable)
		{
			//4.3 Set drawRect.h to transorigin.y + the value you get from the curve. Use GetActualPoint with a slow middle curve.
			
			//----
		}
		else
		{
			//4.4 Set drawRect.h to transorigin.y - the value you get from the curve. Use GetActualPoint with a slow middle curve.
